...
二つ目の同期対象が生まれる。ゲートの規則は ADR-0073（シェルは文面ではなくレーンが
決める）で修正された形で次のとおり:
...
1. **到達範囲はカーネルが縛る = sandbox-exec** — `shell_exec` はモデルが宣言する
   Seatbelt レーン（`read` / `write` / `operator`、ADR-0073）で走る。`read` レーンは
   専用 scratch にしか書けずネットワークにも IPC にも届かない。`write` レーンは後続
   セッションが信頼するファイルに触れない。プロファイルは起動時にその機体で検証する。
   何が確認を求めるかはコマンドの文面ではなくレーンが決める。ヒューリスティックな
   パス（および文面）検証では動的構築（コマンド置換等）を防げないという agent-skeleton
   外部レビューの指摘への構造的回答であり、文字列規則は判定を上げることしかできない
   Block の床としてだけ残る
...
3. **ツール出力・ファイル内容の隔離** — nlk/guard のノンスタグ XML ラップで
   ...
   ラップの外側でモデルに届くものは出所で決まり、内容では決して決まらない: ゲート
   拒否（ADR-0060）、ラップされた結果の後に付くリモートツールの同一失敗の反復に
   ついてのランタイム注記（ADR-0075）、操作者がインストールした指示としての skill
   本文（ADR-0010）
