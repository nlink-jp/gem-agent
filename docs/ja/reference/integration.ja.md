...
（ツール別に緩和可 — [承認](approval.ja.md)参照）。失敗したコールは出所つきで
描画されます — サーバー自身のエラー・サーバーの拒絶・gem-agent が完了できなかった
コール — そしてターン内の連続 3 コールに同一のエラー文で答えたサーバーは、
調査ではなくあなたへの報告をモデルに求めるランタイム注記で名指しされます
（ADR-0075。3 形は[ツール](tools.ja.md)、`mcp_fault` レコードは
[セッション](sessions.ja.md)参照）。タイムアウトした
呼び出しはサーバー子プロセスを kill し（MCP の `notifications/cancelled` は
受信側が無視してよいので gem-agent は送らず、kill で確実に解除します）、次回
呼び出しで遅延再起動します。
