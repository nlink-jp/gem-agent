...
ヘルパを通る。バケツは web の 2 source 以外では構造的にゼロである。

この修正にあたり「すべてのモデル呼び出しがレコードを残す」を再検証
したところ、残していないものが 1 つ見つかった: `/riskbook learn` は
summary モデルで下書きを生成する（ADR-0050、ADR-0057 の 4 日前に
着地）が、in-memory の tally にしか流していなかった。source
`riskbook_learn` の `usage` レコードを書くようにした — 新しい約束では
なく、ADR-0057 の約束である。
...
### 4. `/usage` はツール結果を名指しし、他は変えない

- **`/usage`。** ツールごとの行に、バケツが非ゼロのとき `· tool results N`
  が付く。初稿は明細を「一瞥」として据え置いたが、レビューが §5 の
  実測値を指した — `web_fetch` の呼び出しの 9 割が取得ページである。
  ツールの入力の 9 割を省く行は一瞥ではなく、桁で間違っている。
  `UsageStats` と終了レシートは不変 — 主ループに組み込みツールはなく、
  そこではバケツは構造的にゼロ。
