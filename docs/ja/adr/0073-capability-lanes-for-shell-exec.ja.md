...
行。サンドボックスが無いとき（§5: 設定で有効なのに適用できなければ起動エラー、
`--no-sandbox` は非封じ込めモード）、全シェルコールは操作者だけのもの。
...
`/private/tmp` 書込・拒否プログラムの起動 — と成功すべきプローブ 2 つ（scratch
書込・プログラム実行）に当てる。
...
### 6. 独立検証パス（2026-09-05）と改訂

独立コンテキストのレビュワー 4 体が実装を読んだ（実プローブによるサンドボックス層、
450 行のモード × レーン行列による判定経路、有界 I/O 移行と文書の全主張、組織の
記録済み教訓との突合）。所見 38 件。設計を変えた・穴を閉じたもの:

- **端末に届いていた（高）。** 子プロセスが操作者の制御端末を保持し、`/dev/tty` を
  読取で開いて `TIOCSTI` で gem-agent 自身のプロンプトに打ち込めた — read レーンから
  `!command`・`/auto`・`y` を。全レーンで端末デバイスの `file-ioctl` と `file-read*`
  を拒否し、ランナーはコマンドを独自セッション（`Setsid`）で起動するので `/dev/tty`
  はそもそも解決しない。検証はこれを開いて失敗を期待する。
- **`.git` 自体（高）。** write レーンは `.git` *配下*の書込を拒んだが、`.git` を
  改名して仕込んだフック付きで戻すこと、`gitdir:` ポインタファイルで置き換える
  ことは拒まなかった。`.git` エントリを永続一覧に加える。**残留、記録:**
  *サブディレクトリ*の永続ファイルは親ディレクトリの差し替えで置き換えられる
  （rename はディレクトリ自身のパスしか検査しない）。gem-agent はルートと祖先の
  指示ファイルを読むので、これが届くのは入れ子の `.claude/`・入れ子リポジトリの
  フック・他エージェントの入れ子 `CLAUDE.md`。構造的修正 — 永続ファイルの
  ダイジェストを記録し、変化したものは操作者確認まで拒む — は ADR-0074。プロジェクト
  自身の親ディレクトリは write レーンで決して書けないので、E2E 環境で可能だった
  プロジェクト丸ごと差し替えは閉じた。
- **検証がプローブ 2 つで空虚だった。** `kill -0 1` と port 9 への接続は sandbox
  の有無に関わらず非 root では失敗する。プローブはこのプロセスが開いた loopback
  listener への接続と自分の親へのシグナルに替え、失敗すべき各プローブはまず
  sandbox 無しの対照実行で*成功*することを確かめる。
- **read の到達範囲（設計レビュー V2）。** read レーンは書込を縛ったが全マウントを
  読めた — ADR-0070 §3 が Review にした `/` からの走査が無確認で走っていた。
  `/Volumes`・`/Network`・`~/Library`（ツールチェーン用を除く）を拒否する。本 ADR は
  read レーンについて ADR-0070 §3 を承知の上で反転する: 走査を縛るのは文字列規則
  ではなくカーネル。
- **read レーンの環境。** 素の `env` が操作者の export したトークンをトランスクリプト
  に印字していた。トークン・キー・パスワードらしい名前の変数は read レーンの
  コマンドに継承しない。
- **ヒアドキュメント。** システムのシェルは `TMPDIR` が何であれ `/private/var/tmp`
  にヒアドキュメントファイルを作る（ディレクトリより狭い規則では通らないことを
  プローブ）。この共有ディレクトリ 1 つが「共有物には書かない」の文書化された例外
  で、コマンド自身の記述子の `/dev/fd` と並ぶ。
- **操作者の目の前のレーン。** 承認ボックス・⚙ 行・`gate_decision`/`auto_decision`
  記録・`approval.decision` telemetry がレーンで始まる（`[write] cmd`・
  `[unverified:read] cmd`・`[unconfined:write] cmd`）。レーンを名指ししない `access`
  値は read 扱いで通さず、どのゲートの前でも拒否する。
- **`never` ポリシーや `--allow shell_exec` 下**では write レーンがネットワーク込みで
  無人で走る — リファレンス文書は逆を述べ、シェルに `--auto` より `--allow` を
  勧めていた。訂正。挙動は ADR-0008 のもの: `never` は操作者の明示的な緩和。
- **オプトアウト。** `[sandbox].read_lane_prompts = true` で read レーンのコマンドも
  確認する（檻はそのまま）— 破壊的変更手順が求める互換オプション。
- **アーキテクチャテスト**は import を解決するので、別名・dot-import・関数値・
  `bounded` という名の変数で呼出しを隠せない。ルール層を import できるのは
  `internal/agent` だけ。
- **有界 I/O の残留**: `@` の予算枯渇で記述子がリーク、ちょうど上限の指示・メモリ・
  スキルファイルが rune の途中で切れる、`--continue` が打ち切られた一覧で推測、
  フック出力が無言で切れる。すべて修正。

レビュワーの最終統合報告（3 件、いずれも攻撃成立の実証ではない）でパスを閉じた:

- **R1 — 検証プローブが固定名に書いていた。** プロジェクトに既存の
  `.gem-agent-lane-probe`（例えば symlink）が、sandbox 無しの対照実行で参照先ごと
  上書きされ、その後削除された。プローブはこのプロセスが排他的に作った
  ファイル（`O_EXCL`・ランダム名）にだけ書き、それだけを削除する。
- **R2 — 別名の先にある永続ファイル。** `AGENTS.md` への symlink／ハードリンク
  `notes.md` への `write_file` は名前で Safe と判定され、リンク越しに書いた。
  構造的な変更 2 つ: 判定は実パス（`Registry.RealPath`、ルート内で symlink 解決）で
  行い、file ツールはその場で書かない — 隣に新ファイルを作って名前の上に rename
  するので、書込が別名から届く inode に着地することはない（ハードリンクは新しい
  ファイルになり、リンク元は元のバイトを保つ）。
- **R3 — 共有ルート配下の state ディレクトリ。** 作業ディレクトリを
  `/private/var/tmp` 配下に置くと、read レーンのヒアドキュメント用 allow が覆った。
  プロジェクトと作業ディレクトリは全共有 allow の*後*に名指しで deny し、専用
  scratch だけを最後に再 allow する。共有ルート配下の作業ディレクトリはテストで固定。

記録のみ: `ps`/`top` はどの Seatbelt プロファイルでも走らない。pty 確保（`script`・
`expect`）は全レーンで失敗。`.gitmodules`・`.gitattributes` は write レーンで書ける
（どちらも操作者自身の git コマンド無しには実行されない）。一覧外の資格情報の
場所は読め、write レーンでは送れる — そこの判定者はモデル層か操作者。

### 7. 拘束そのものも測定する（2026-09-05、v0.70.2 後）

§5 は read レーンを検証済みの主張にしたが、`Confined` — sandbox-exec が何らかの檻を
適用しているという事実 — は設定に委ねていた: バイナリが存在しプロファイルが生成できれば
confined。非公式移植（sandbox 無しで WSL 向けに再ビルドしたソース）の報告がその穴を示した:
sandbox-exec の存在確認をスタブ化したビルドや、プロファイルを無視する別物の `sandbox-exec`
は confined として通り、read レーンのプローブには落ち、全コマンドを *write レーン*の呼出
— Review 層、auto モードならモデル層が承認できる — として走らせる。背後にカーネルは無い。
これは操作者専用になる `--no-sandbox` より弱い。

`VerifyWriteLane` が起動時と `/clear` で `VerifyReadLane` と並んで走る: 実 write レーンの
プロファイル下で、プロジェクト内の指示ファイル名への書込と、操作者のホーム直下（この
レーンが常に拒否し、プロジェクトでも作業ディレクトリでも scratch でもない唯一の場所）への
書込は、sandbox 無しの対照実行が成功した後に失敗しなければならず、通常のプロジェクト
書込とプログラム実行は成功しなければならない。プローブファイルはランダム名で排他作成し
削除する。どれかが満たされなければセッションは**未確認**: `Enforcement{}` — 全シェル
コマンドは操作者が答え、バナーが理由を告げ、コマンドはラップしたまま（半分効く檻でも
邪魔にはなる）。`/status` は設定ではなく測定した状態を示す。ここで誰も検証していない
ビルドに対する最後の安全弁になる: 保護はモデルへ静かに落ちるのではなく、人間へ落ちる。

## 検討した代替案

- **文字列層を残し、欠けた綴りを足す。** 却下: 領域が無限（9 パス・所見 20・再発 2）。
- **プログラム拒否一覧を read レーンの根拠にする。** 設計レビュー後に却下:
  プログラム一覧は別名の正規表現。根拠はケイパビリティ族、一覧は第 2 線。
- **既定モードでも全 `shell_exec` を確認したままにする。** 利用者が却下: カーネルは
  確認ダイアログより強い保証で、`ls` の確認がセッションを auto モードへ追いやった。
  代わりに `[sandbox].read_lane_prompts` として提供。
- **ADR-0070 §3 の「`/` からの走査」規則を昇格専用の文字列規則として残す。**
  カーネルで read の到達範囲を縛る（`/Volumes`・`/Network`・`~/Library`）方を採用。
- **コンテナ隔離。** 範囲外: gem-agent は macOS 専用で操作者自身のツールチェーンを
  走らせる。Seatbelt がプラットフォームの檻。

## 参照

- ADR-0001（サンドボックス機構）、ADR-0004（ラダー）、ADR-0008（`never`/`always`）、
  ADR-0047（宣言は表示し信頼しない）、ADR-0053（`--allow`）、ADR-0070 §2（共有一覧
  1 つ）、ADR-0072（所見）
- macOS 26 で実測した Apple Seatbelt の SBPL 操作（§背景・§5・§6）
- 2026-09-05 の独立レビュー報告 4 本（リポジトリ外に保管）
