...
- [`ADR-0080`](adr/0080-read-only-is-a-mode.ja.md) — read-only はモードであり、独立した軸である（**Accepted**、決定済み・未実装）。散文だけでは 2 通りに破れる（2026-09-09、`gemini-3.8-flash` で計測）: 後半ターンの `make build` は入力が「続けて」なら 0.95 で承認され、同一コールでも制約が見えていれば 0.95 で制約を名指してエスカレートする — 弱いのはモデル層ではなく、制約がターンを越えないことだけである。さらに `write_file` は制約が*まさにそのターン*にあってもモデル層が走らずに承認される（プロジェクト内書込はルール層 `safe`）。ADR-0077 は 1 段下で同じ線を引いている: ゲートが言うのは**いつ訊くか**であって**このセッションに何があるか**ではない。`--auto` は誰がゲートに答えるかを決め、こちらはセッションが何に到達できるかを決める。結合すれば、最も必要な 2 つのモードで制約が無効になる — 手動（さもなくば操作者はコール 1 件ごとに*いいえ*と答える）と `-p`（答える者がいない）。軸はセッションの**レーン上限**（コールは宣言と上限の低い方で走る）で、状態は 3 つ: **off**（`operator`、今日どおり、コスト無し）、**read-only**（`read`）、**auto read-only**（ランタイムが自分で `read` へ締め得るが、**決して**緩めない — 締めるのは拒否を増やすだけだが、緩めるのは導出された制約が導出された許可になる方向である）。既定は `[agent].read_only` が決め、`--writable` / `--read-only` / `--auto-read-only` が実行単位で上書きする。`-p` の形は 2 つだけである — そこで上限を推定すれば cron ジョブの書込可否がその prompt の文言の関数になるからで、設定の `"auto"` は `"on"` として読む（無視すれば、誰も見ていない場所で制約を黙って落とす）。強制は `LaneRead` で、それは既に要求されたプロファイル**である**うえ起動時にプローブ検証される唯一の境界でもある: モードはプロファイルを差し替えるのではなくレーンを上限で抑えるので、新しく生成するものが無く、間違えるものも無い。書込は解除するか訊く — must-prompt で `a`・`never`・モデル層は答えられず、断ればそのターンの解除プロンプトを抑止し、`-p` では拒否が最終である。そしてモードは、強制としてではなく操作者の**意図**としてリスク評価器へ届く。これがプロファイルの届かない MCP を覆う: 計測では、他では依頼に沿って承認される vault 書込を 0.95 承認から 0.95 エスカレートへ反転させ、外部送信を 1.00 でエスカレートさせ、検索は 1.00 で承認のまま残し、しかも状態を述べる文言（verdict は同一だった）と違って、何も妨げていない書込を「not permitted」と操作者に告げさせることが無い
- [`ADR-0081`](adr/0081-instruction-context-only-subtracts.ja.md) — 指示文脈は減算しかしてはならない。頼むのではなく合成で（**Accepted**、未実装、ADR-0080 から分離）: 評価器の指示文脈は今日すでに双方向である — ADR-0038 は整合が承認を支持すると書き、実際にそうする様子が計測された（「続けて」としか言っていないターンで、後半ラウンドの `make build` を 0.95 の承認まで運んだ）。経路は操作者の打鍵テキストであり攻撃者が直接書くことはできないが、ログやエラーメッセージを貼った操作者は、このシステムが信頼する唯一の入力に攻撃者のバイト列を持ち込んでいる。それは 1 ターン幅でも既に真である。よって `approve = （指示文脈なしで到達した判断） AND （それを与えた評価）`: `safe` はラウンドを走らせない（ルール層が文脈なしの判断**である**うえ、指示文脈はそれらのコールに届かないので合成するものが無い — ADR-0080 第 2 稿から引き継いだ記述を実装時に訂正）、baseline がエスカレートすれば 1 ラウンド（文脈に救済を許さない以上、aligned を走らせる意味が無い）、それ以外は 2 ラウンド。baseline は文脈非依存なのでツールと引数でキャッシュできる。プロンプトへの指示ではなく合成にするのは、「整合はエスカレーションの根拠にしてよいが承認の根拠にするな」が文字列規則であり、それを無視するモデルに対しても成り立たねばならないからである — ADR-0020 §4 がメモリ書込をモデル層から外し「気をつけろ」と言わなかったのと同じ理由。下限は ADR-0038 以前の評価器であり、ADR-0080 §5 が探して見つからなかった「檻があるから承認してよい」の読みも縛る。意思変更は影響を受けない — 解除に文脈が承認を*作る*ことは一度も必要ないからである。未計測: 本 ADR が止めようとしている事例を構成したプローブはまだ無い
