...
- [`ADR-0079`](adr/0079-a-notice-is-chrome-an-error-is-a-chain.ja.md) — 通知はクロムであり、エラーは鎖である: エージェントがターン中に書く通知 9 本を、ADR-0029 §3 が既に覆っているという読みで ADR 無しにカタログへ移したが、覆っていたのは 4 本で、残り 5 本は閉じた列挙に入る新しい面だった。さらに、移動を正当化するために書いた規則「操作者の次の一手を運ぶ文はクロム」は自分の所属を決められない —— 要約失敗の文字列はコマンドを運ばないのにカタログにある。判定基準は著者性である: gem-agent が文を組み立てる → カタログ（原因はそのまま引用してよい）、文そのものが返される `error` → 英語。ライブラリの中身に翻訳された枠を被せることが §3 の消した混合だからである。次の一手の規則は文言についてのものであり、言語についてでは無かった
