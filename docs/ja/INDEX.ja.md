...
- [`ADR-0080`](adr/0080-read-only-is-a-mode.ja.md) — read-only はモードである: 推論が提案し、操作者が宣言し、レーンが強制し、レーンの届かない先へは評価器が運ぶ（**Proposed**）。散文だけでは 2 通りに破れる（2026-09-09、`gemini-3.8-flash` で計測）: 後半ターンの `make build` は入力が「続けて」なら 0.95 で承認され、同一コールでも制約が見えていれば 0.95 で制約を名指してエスカレートする — 弱いのはモデル層ではなく、制約がターンを越えないことだけである。さらに `write_file` は制約が*まさにそのターン*にあってもモデル層が走らずに承認される（プロジェクト内書込はルール層 `safe`）。よって制約はセッション**状態**にする: 手動で切り替え、ステータス行に出し、状態だからターンを越える — 散文をターン境界を越えて運ぶ必要も、文脈窓を広げる必要も無い。推論に残す権能はただ 1 つ、「read-only に切り替えますか」と**訊く**ことだけであり、緩める方向に誤り得た導出は質問に変わる。断られたことは記憶し、二度は訊かない。gem-agent の届く範囲の強制は `LaneRead` で、それは既に要求されたプロファイル**である**（私有スクラッチ以外へ書かず、ネットワークも無い）うえ、起動時にプローブ検証される唯一の境界でもある: モードはプロファイルを差し替えるのではなくレーンを上限で抑えるので、新しく生成するものが無く、間違えるものも無い。file ツールはセッション作業ディレクトリの外を拒否し、メモリ書込は拒否する。書込の試みはモードを解除するか訊く — must-prompt であり `a`・`never`・モデル層のいずれも答えられず、断ればそのターンの解除プロンプトを抑止するので、汚染されたツール結果がプロンプトで操作者を押し切れない。そしてモードは、強制としてではなく操作者の**意図**としてリスク評価器へ伝える。これが Seatbelt プロファイルの届かない MCP を覆う: 計測では、他では依頼に沿って承認される vault 書込を 0.95 承認から 0.95 エスカレートへ反転させ、外部送信を 1.00 でエスカレートさせ、検索は 1.00 で承認のまま残した。状態を述べる文言でも verdict は同一だったが、モデルに書込を「not permitted」と言わせた — MCP では偽であり、しかも操作者に表示される。§3 はカーネルの拒否、§5 は判断であり、両者を曖昧にしてはならない
