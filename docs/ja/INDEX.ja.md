...
- [`ADR-0080`](adr/0080-restriction-outlives-the-turn.ja.md) — 操作者が述べた制約はターンを越えて生き、指示文脈は減算しかしてはならない（**Proposed**）: 2026-09-09 に `gemini-3.8-flash` で計測したところ、「読み取りだけで、変更はしないで」は 2 通りに破れる。後半ラウンドで提案された `make build` は、そのターンの入力が「続けて」だと 0.95 で承認され（"Standard build command within project directory"）、同一のコールでも制約が見えていれば 0.95 で制約を名指してエスカレートする — つまり弱いのはモデル層ではなく、制約がターンを越えないことだけである。さらに `write_file` は、制約が*まさにそのターン*にあってもモデル層が一度も走らずに承認される。プロジェクト内書込がルール層 `safe` だからである。「対話履歴を入力し続ける」のは方向が逆である: 履歴には出所が 3 つあり、継続する制約もその撤回も操作者が打った文にしか無く、そこは injection の攻撃者が書けない唯一の経路である一方、モデル自身の発話は ADR-0047 §3 が剥がした自己正当化を復活させ、ツール結果は攻撃者が直接書ける。この分離はコード上に既にある（`turnInput` の代入は 1 箇所で `Run` から来る。`!` 出力は `AddContext` 経由で入る）。ただし今日の指示文脈は*双方向*であり — ADR-0038 は整合が承認を支持すると書き、M1 はそれを実測した — 窓を広げる前にその影響を減算専用にしなければならない: `approve = （指示文脈なしで到達した判断） AND （打鍵文脈を与えた評価）`。`safe` では 1 ラウンド（ルール層が文脈なしの判断**である**）、baseline がエスカレートすれば 1 ラウンド、それ以外は 2 ラウンド。プロンプトへの指示ではなく合成にするのは、この性質が指示に従わないモデルに対しても成り立たねばならないからである。下限は ADR-0038 以前の評価器であり、意思変更は依然として通る — 解除に文脈が承認を*作る*ことは一度も必要ないので、解除経路も自動解除も持たずに済む。`--read-only` は別の問いに答えているとして却下。制約オブジェクトの導出も却下するが、理由は「導出された許可になるから」ではない（`safe`→`review` だけの導出物は単調であり、初稿の論法は誤りだった）。§1 が、正しく保つべき新しいオブジェクトなしに同じ場所へ到達するからである
