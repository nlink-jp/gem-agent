...
- [`ADR-0076`](adr/0076-the-configuration-home-stays-readable.ja.md) — **（提案）** 操作者の設定ホームはレーンから読めるままにする: 初稿は `~/.config` を拒否したが、そのレビューで 1 つの deny が再許可規則 4 つ・順序規則 2 つ・symlink 解決・操作者が維持する一覧を生むことが分かった — カーネルは syscall を見て意図を見ず、read レーンの存在理由であるツールがそこの設定を読む。挙動は代わりにその契機（ADR-0075）で扱う。ADR-0011 §3 修正: Claude Code 向けにインストール済みの skill はリンクせずコピーする — `~/.claude` はレーンが読めない資格情報の置き場で、Seatbelt は解決後のパスで照合する。`/skills` の空状態はコピーコマンドを表示
