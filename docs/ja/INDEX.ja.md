...
- [`ADR-0080`](adr/0080-what-arming-auto-delegates.ja.md) — auto を武装するとき何を委任するのか、そしてゲートが答えられない拒否（**Proposed**）: レビューの 3 指摘は 1 つの非対称である — 許可は継続するセッション状態で、禁止はそうではない。ルール層の `safe` が階梯を終わらせるので「評価だけ、書き込み禁止」で始めたセッションにはプロジェクト内の通常編集を止める層が無い。評価器から見える意図は `a.turnInput` の 1 ターン分で、しかも先頭優先でクリップされるため「続けて」は何も運ばず、最終段落の禁止事項は落ちる。そしてモデル層が拒否したコールに、別のコールについて以前打った `a` が答える — `a` が同じ名前への以後の任意引数を覆う MCP で最も広い。`risk.Classify` の手前で設定を検査するだけでは見せかけである — ADR-0077 §1 は既に、`write_file` を取り除くと作業が `shell_exec` とリダイレクトへ移るとして組み込みの選択を拒否している — ゆえに強制点は**レーン**である: `[scope] writes = "off"`（および `--read-only`）はシェルを `read` レーンに縛り、それは起動時に「失敗すべきプローブ」で検証される唯一の境界であり、変更系 file ツールはゲート以前に拒否される。スコープ外はエスカレーションではなく拒否であり（allowlist が答え得るスコープはスコープではない）、Block 層でもない（操作者は既に否と言っている）。モデル層の拒否はそのコールを must-prompt にするが、評価の*失敗*は意図的にそうしない — 不安定な回線は操作者をプロンプトを消すよう訓練する — そして両者は `auto_decision` が `confidence` を持つようになったので区別できる。パス単位の書込スコープは名指すが作らない: `write` レーンのプロファイルが同じ一覧から生成されるまで何の価値も無いからである。制約をモデルに散文から抽出させることはしない: 提案者が自分の檻を書くことになり、導出が緩める方向に外れた瞬間、導出された制約は導出された許可になる
