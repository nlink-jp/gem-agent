...
`rejected the call …`, or `gem-agent could not complete …`) and a note
outside the nonce tag tells the model that its arguments went out as
written, that the answer repeated, and to report to you and ask how to
proceed. You see one line per streak; nothing is retried on your behalf.
...
|---|---|---|
| `read` | every write except into the session's private scratch directory (`TMPDIR` points there, and so does every cache in `[sandbox].scratch_caches` — Go's `GOCACHE` by default, one directory for this lane and a separate one for the approved lanes — so `go build`, `go vet` and `go test` run in this lane, ADR-0084), the shells' here-document directory `/private/var/tmp`, its own descriptors (`/dev/fd`) and the device sinks — the project, the work directory and `/private/tmp` included; the network; Mach and POSIX IPC, Apple Events, launch services (`open`), preference writes (`defaults`), NVRAM, device access, the terminal, signals to anything but the command's own children; reading the credential list (below), external and network mounts (`/Volumes`, `/Network`) and your `~/Library` except the toolchain directories (`Caches`, `Developer`, `Python`, `Go`); your exported variables are **not** filtered, in this lane or any other (ADR-0087) — which one a program needs is not a question gem-agent can answer, so the environment it was launched with reaches every child it spawns, this shell, MCP servers and hooks alike; what no child gets is gem-agent's own configuration variables (`GEMAGENT_PROJECT`, `GEMAGENT_LOCATION`, `GEMAGENT_MODEL`, `GEMAGENT_STATE_DIR`, `GEMAGENT_MCP_STDERR`), while the three it exports for children (`GEMAGENT_WORK_DIR`, `GEMAGENT_SESSION_ID`, `GEMAGENT_PROJECT_DIR`) pass — so a companion you run from a shell lane that wants gem-agent's own state root (`gem-usage-lens`, say) takes it from its own flag or configuration rather than from the environment; launching the IPC-capable programs (`osascript`, `open`, `launchctl`, `defaults`, `security`, `pbcopy`, `shortcuts`, `automator`, `scutil`, `networksetup`, `systemsetup`, plus `[sandbox].read_lane_deny_exec`) as a second line | nobody — a read-lane command changes nothing but its own scratch and **runs without a prompt** in every mode except under an `always` policy, like `read_file` — on a machine where the lane's denials were verified at startup (below), unless you set `[sandbox].read_lane_prompts = true` |
...
The status line carries one badge holding both: the padlock is the
ceiling right now, the word is the mode.
