...
(stdio JSON-RPC client), `internal/banner` (the lines printed before the operator has typed
anything, and the rule that decides which ones — ADR-0078),
`internal/mcpfilter` (the one predicate behind
