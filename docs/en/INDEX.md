...
- [`ADR-0089`](adr/0089-inline-images-declare-their-height.md) — inline images declare their box: the counter is told, never measures (**Proposed**, not implemented; rewritten 2026-09-17 and corrected again the same day, after two verification passes): `emit` counts the physical rows of every line and the bottom pin rests on that count, but against x/ansi v0.11.6 `ansi.StringWidth` is 0 for iTerm2 `OSC 1337`, kitty `APC _G` and sixel `DCS q` alike, and `physicalRows` floors at 1, so the shortfall is N-1. `tools/pinprobe` drives the real model through the real emit path, `-drive` runs the experiment and prints the table it measured, and the regime is arranged from the terminal's own height — a filler count chosen for a 30-row tmux pane had put an 80-row iTerm2 window on the not-full side of the pad branch, and the first rewrite reported those runs as "full" and concluded that iTerm2 was unharmed. Re-taken in the arranged regime, each with a plain control at the same fill: iTerm2 3.7.2 strands three frames for three images, tmux 3.7c strands three for three rendered sixels with the gap widening by picture height, the payloads that tmux swallows stay clean in the same run, and a screen not yet full takes no damage anywhere. So the emitter declares the box and `physicalRows` is told it, replacing its floor of 1; the declaration covers COLUMNS too, though for the row count's sake rather than the renderer's, since Bubble Tea gates on `ansi.StringWidth` and is blind in the same way. The source is ONE entry and it is BYTES, never a path: the MCP intake's `write` short-circuits on `os.Stat`, and a server that knows its own name and the bytes it will return can plant a symlink at the content-addressed path, so a view-layer open would walk back into the class ADR-0085/0086 repaired — the very class a model-named path is rejected for. Restored after being wrongly withdrawn: the organization's supply-chain rule, which is written in the sibling runtime's RFP and its `internal/llm`, and against which `mermaid-ascii` is a pre-policy dependency rather than a counter-example. Left unmeasured and said so: kitty and Ghostty honouring `r=`, Terminal.app's support, the per-image cost
