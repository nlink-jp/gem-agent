...
- [`ADR-0089`](adr/0089-inline-images-declare-their-height.md) — inline images declare their height: the counter is told, never measures (**Proposed**, not implemented): the operator asked whether a graphics-capable terminal can draw inside a reply. `emit` counts the physical rows of every line it prints and the bottom pin rests on that count, but an image is not a wide line — it is a line the counter cannot see: measured against x/ansi v0.11.6, `ansi.StringWidth` is 0 for iTerm2 `OSC 1337`, kitty `APC _G` and sixel `DCS q` alike, while `ansi.Hardwrap` leaves all three byte-identical, so `wrapForScrollback` shears nothing. `tools/rowprobe` then measured the terminal itself (iTerm2 3.7.2, 16/16 cursor reports): the declared box is reserved exactly in both dimensions whatever the picture does inside it — a 16:9 image in a 40x12 box draws about ten rows and occupies twelve, and the 40x6 cases end at column 41 though the drawing is 24 columns wide — so no aspect-ratio derivation is needed; the cursor is left on the image's LAST row, which makes the raw delta one short and made a terminal honouring every declaration read as one honouring none on the first pass; and an undeclared image takes its native size, recoverable only from the cell pixel size. Hence the emitter declares the height and `physicalRows` is told it, with no second path to disagree. An image gets its own line (text beside one splits across its first and last rows), goes out verbatim on ADR-0063's art lane, and reaches only the two protocols that can declare a row count — sixel is refused for needing a derivation and a community encoder. The capability is probed once before Bubble Tea owns stdin (v1 does not decode kitty/CSI-u, and an abandoned cursor report is misfiled into the next query, not lost: 11 sent, 5 read, 6 landing on the shell prompt after exit), selected by `[tui] images`, off inside a multiplexer unless forced. What may be drawn is a two-entry list, not a rule; only the view layer emits an image escape, and the prompt says nothing, as ADR-0063 §2 requires. Unresolved and written down rather than claimed: iTerm2 answered the next cursor report 0.8–1.5 s after a 2.4 KB payload, which bounds its parser and not its drawing, and tool output still reaches the terminal without ANSI stripping — pre-existing, not widened, not repaired here
