...
- [`ADR-0089`](adr/0089-inline-images-declare-their-height.md) — inline images declare their box: the counter is told, never measures (**Proposed**, not implemented; three verification passes, and the source question deferred out of it): `emit` counts the physical rows of every line and the bottom pin rests on that count, but `ansi.StringWidth` is 0 for iTerm2 `OSC 1337`, kitty `APC _G` and sixel `DCS q` alike against x/ansi v0.11.6, and `physicalRows` floors at 1, so the shortfall is N-1. `tools/pinprobe` drives the real model through the real emit path, computes its own table, and arranges the regime from the terminal's height — the pad branch that production labels "screen not full" is where nothing is harmed, and a filler count chosen for a 30-row pane had put an 80-row window there while the record called it full. Re-taken with a plain control at the same fill in every run: iTerm2 3.7.2 strands three frames for three drawn images, tmux 3.7c three for three rendered sixels, the payloads tmux swallows stay clean in the same run, and a screen not yet full takes no damage anywhere. So the emitter declares the box, `physicalRows` is told it in place of its floor of 1, and the declaration covers COLUMNS too — for the row count's sake, since Bubble Tea gates on `ansi.StringWidth` and is blind the same way. **What may be drawn is NOT settled here.** That decision was written and refuted three times, each time the worst finding of its round — a model-named path bypasses the credential enforcers, the intake's path can be pre-empted by a symlink because `write` short-circuits on `os.Stat` and every call hands the server the work directory, and the bytes the third draft would carry have no carrier at all, since `render` returns a string and `Tool.Run` is string-only. Three drafts in one place is one mistake: the source cannot be named until the plumbing exists. It is deferred to its own ADR, which inherits the one constraint that held against all three: **the view layer opens no file.** Decision 4 rests on sixel's inability to declare a row count alone, the supply-chain rule having been first wrongly withdrawn and then wrongly extended to an encoder, which its own primary source says it was not written for. Closed by mechanism rather than by another read, in the same commit: a test that resolves every `file.go:NNN` in these records against the symbol its paragraph names, and an absence test over retired claims across 215 read surfaces — the two classes that produced a fresh instance inside each previous round's repair
