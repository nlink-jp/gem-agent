...
- [`ADR-0080`](adr/0080-what-arming-auto-delegates.md) — what arming auto mode delegates, and the refusal the gate cannot answer (**Proposed**): three review findings are one asymmetry — permission is durable session state and prohibition is not. The rule tier's `safe` ends the ladder, so a session opened "review only, do not write" has no layer that stops an ordinary in-project edit; the evaluator's picture of intent is `a.turnInput`, one turn wide and clipped head-first, so "続けて" carries nothing and a prohibition in the last paragraph is dropped; and a call the model tier refused is answered by an `a` typed earlier about a different call, widest for MCP where `a` covers any later arguments. Testing a setting ahead of `risk.Classify` is theatre on its own — ADR-0077 §1 already refused to make built-ins selectable because removing `write_file` moves the work into `shell_exec` and redirection — so the enforcement point is the **lane**: `[scope] writes = "off"` (and `--read-only`) holds the shell to the `read` lane, the one boundary verified at startup against probes that must fail, and refuses the mutating file tools before any gate. Out of scope is a refusal, never an escalation (a scope the allowlist can answer is not a scope) and never Block-tier (the operator has already said no). A model-tier refusal makes that call must-prompt, an evaluation *failure* deliberately does not — a flaky network would train the operator to clear prompts — and the two are now distinguishable because `auto_decision` carries `confidence`. Path-scoped writes are named and not built: they are worth nothing until the `write` lane's profile is generated from the same list. Constraints are never extracted from prose by a model: the proposer would author its own cage, and a derived constraint is a derived permission the moment it errs toward loosening
