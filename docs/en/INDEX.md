...
- [`ADR-0080`](adr/0080-read-only-is-a-mode.md) — read-only is a mode: proposed by inference, declared by the operator, enforced by the lane, and carried to the evaluator where the lane cannot reach (**Proposed**). Prose alone fails twice, measured 2026-09-09 against `gemini-3.8-flash`: a `make build` at a later turn whose input is 「続けて」 is approved at 0.95 while the identical call with the restriction in view escalates at 0.95 naming it — the model tier is not the weak part, the restriction just does not survive the turn — and a `write_file` with the restriction in *that very turn* is approved with the model tier never running, because an in-project write is rule-tier `safe`. So the restriction becomes session **state**: toggled by hand, shown in the status line, and surviving turns because state does, which is why no prose has to be carried across the turn boundary and no context window widens. Inference keeps exactly one power — to **ask** "switch to read-only?" — so a derivation that could have been wrong in the loosening direction becomes a question instead, and a decline is remembered rather than re-asked. Enforcement inside gem-agent's reach is `LaneRead`, which already **is** the requested profile (no writes outside its private scratch, no network) and is the one boundary probe-verified at startup: the mode caps the lane rather than swapping a profile, so nothing new is generated and nothing new can be got wrong; the file tools refuse outside the session work directory and memory writes refuse outright. A write attempt asks whether to lift the mode — must-prompt, answerable by no `a`, `never` or model tier, and a decline suppresses further lift prompts for that turn so a poisoned tool result cannot grind the operator down. And the mode is stated to the risk evaluator as the operator's **intent** rather than as enforcement, which is what covers MCP, where no Seatbelt profile reaches: measured, it flips an otherwise-aligned vault write from approve 0.95 to escalate 0.95, escalates an external send at 1.00, and leaves searches approved at 1.00. State-shaped wording produced identical verdicts but made the model say a write was "not permitted" — false for MCP, and shown to the operator. §3 is a kernel denial and §5 is a judgment, and the two must not be blurred
