# Documentation Index

Entry point for gem-agent's maintainer-facing documentation. For
user-facing material see [`README.md`](../../README.md).

Japanese mirror: [`INDEX.ja.md`](../ja/INDEX.ja.md). `scripts/docs-mirror-check.sh`
enforces the structural half in `make check` — every `docs/en` file has its
`docs/ja` counterpart and back, the ADR catalogue is complete and ordered in
both, and the code spans of each pair agree. Prose parity is the author's job.

## Specification

- [`gem-agent-rfp.md`](gem-agent-rfp.md) — the canonical spec: problem
  statement, functional surface, scope boundaries, phase plan. Features
  outside it need an ADR, which is how session resume and context
  compaction got in.

## Reference

Current behaviour. Evergreen — updated in place as the code changes.

Feature references (the README links here; one domain per file):

- [`reference/interface.md`](reference/interface.md) — TUI, plain REPL,
  one-shot, keys, slash commands, completion, `/settings`, theme and
  UI language
- [`reference/tools.md`](reference/tools.md) — every built-in tool and
  the design decision behind it
- [`reference/attachments.md`](reference/attachments.md) —
  @-references: files, images, documents, audio/video, the GCS route
- [`reference/approval.md`](reference/approval.md) — MITL gates,
  auto-approve, the per-tool policy, sandbox, startup safety,
  untrusted-content isolation
- [`reference/sessions.md`](reference/sessions.md) — transcripts,
  resume, state layout, compaction, `/usage`, agent memory
- [`reference/integration.md`](reference/integration.md) — project
  instruction files, MCP servers, skills
- [`reference/configuration.md`](reference/configuration.md) — install,
  the config file, precedence, flags, telemetry, content filters,
  endpoints

Project references:

- [`reference/architecture.md`](reference/architecture.md) — package
  layout, the turn loop, the two confinement boundaries, persistence,
  and failure behaviour in one table
- [`reference/drill.md`](reference/drill.md) — the on-demand health
  check (the former monthly drill, ADR-0061): what rots on its own, the
  procedure that catches it, and the record of the first run
- [`reference/promotion.md`](reference/promotion.md) — closed record of
  the lab-series → cli-series bar and the 2026-09-01 promotion decision
  that superseded it (ADR-0061)

## ADRs

Point-in-time design decisions. Immutable once accepted; a changed
decision gets a new ADR that supersedes the old one (typo and link fixes
excepted).

- [`ADR-0001`](adr/0001-sandbox-mechanism.md) — sandbox-exec plus MITL:
  two boundaries, one for decisions and one for containment
- [`ADR-0002`](adr/0002-tui.md) — Bubble Tea in inline mode; alt-screen
  rejected to keep native scrollback and copy/paste
- [`ADR-0003`](adr/0003-bottom-pinned-layout.md) — pinning the input to
  the window bottom without alt-screen
- [`ADR-0004`](adr/0004-auto-approve.md) — auto-approve as a two-tier
  ladder: a rule floor the model cannot lift, then a model judgement
- [`ADR-0005`](adr/0005-session-resume.md) — the session log becomes the
  resume source of truth; refusals over warnings on project and model
- [`ADR-0006`](adr/0006-context-compaction.md) — summarise the older
  half instead of failing at the window; fail safe, never fail small
- [`ADR-0007`](adr/0007-input-during-a-turn.md) — typing during a turn is
  kept and queued; auto-sent only when the turn finished cleanly
- [`ADR-0008`](adr/0008-per-tool-approval-policy.md) — per-tool approval
  policy; a project may tighten freely and loosen only where trusted
- [`ADR-0009`](adr/0009-settings-panel.md) — a settings panel showing
  provenance, and a machine-owned policy file so comments survive
- [`ADR-0010`](adr/0010-skills.md) — Claude Code's skill format read
  as-is; skill content is instructions, bounded by confined reads
  (location clause superseded by 0011)
- [`ADR-0011`](adr/0011-skill-scope-separation.md) — skills live in
  gem-agent's own directory; sharing with Claude Code is a symlink
- [`ADR-0012`](adr/0012-image-input.md) — image input: operator-attached
  via @ (clipboard included), model-viewed via view_image
- [`ADR-0013`](adr/0013-navigation-tools.md) — list_tree and
  search_files: orientation and fast grep, no index, no dependencies
- [`ADR-0014`](adr/0014-context-economy-tools.md) — summarize_file on a
  lightweight model, and line-window reads for read_file
- [`ADR-0015`](adr/0015-edit-file-v2.md) — edit_file v2: batched atomic
  edits, diagnosed misses, evidence on success
- [`ADR-0016`](adr/0016-file-info.md) — file_info: content-judged type,
  metadata, and the MD5/SHA1/SHA256 trio
- [`ADR-0017`](adr/0017-web-tools.md) — grounded search and digested
  fetch; egress-gated by default, SSRF dead by construction
- [`ADR-0018`](adr/0018-context-caching.md) — a session-scoped isolation
  tag makes implicit caching fire; measured 0% → 81–95%
- [`ADR-0019`](adr/0019-usage-accounting.md) — per-category usage
  accounting and /usage; side-calls stop stomping the footer
- [`ADR-0020`](adr/0020-agent-memory.md) — agent memory across sessions:
  two scopes, machine-owned outside the repo, writes are the trust
  boundary
- [`ADR-0021`](adr/0021-review-fixes.md) — whole-code review fixes:
  transcript clear/tear/lock, the allowlist floor, scope-first policy,
  and two refuted-by-measurement findings
- [`ADR-0022`](adr/0022-per-project-session-layout.md) — per-project
  session subdirectories (memory's convention), legacy read in place,
  and GEMAGENT_STATE_DIR isolation
- [`ADR-0023`](adr/0023-startup-safety.md) — startup safety: broad
  roots confirm, and one first-run trust question covers a project's
  instructions, .mcp.json, and skills
- [`ADR-0024`](adr/0024-bottom-hold.md) — bottom-hold: the frame's total
  height is held once the screen is full, so the footer stops bouncing
  (supersedes ADR-0003's full-screen clause)
- [`ADR-0025`](adr/0025-thinking-level.md) — configurable Gemini 3
  thinking level for main-model calls; summary model unaffected,
  supported levels model-dependent (measured)
- [`ADR-0026`](adr/0026-document-reading.md) — document reading: PDF
  native as measured multimodal parts, Office XML extracted locally,
  legacy binaries out of scope
- [`ADR-0027`](adr/0027-audio-video.md) — audio/video input: a
  configured bucket always wins over inline (round-replay economics),
  content-addressed uploads, nothing deleted
- [`ADR-0028`](adr/0028-self-healing-line-counter.md) — the printed-line
  counter follows reality: over-tall frames scroll the terminal, and
  the counter self-heals by the overflow (amends ADR-0003's definition)
- [`ADR-0029`](adr/0029-ui-language.md) — UI language mode:
  `[tui].language` auto/ja/en, one catalog struct with two complete
  literals, completeness enforced by test; log-shaped and model-facing
  text stays English
- [`ADR-0030`](adr/0030-agent-self-info.md) — `agent_info`: a read-only
  self-information tool (model, context occupancy, usage, limits,
  platform); fields earn their place by changing model behavior — GCP
  identifiers and hostname withheld
- [`ADR-0031`](adr/0031-review-fixes-round-2.md) — review round 2:
  ~30 fixes (Msgs wiring, denial bypass, media-store poisoning,
  resume-under-flock, rune-safe completion, adaptive approval budget,
  flocked policy mutations, docext aggregate cap); one 400 claim
  refuted by measurement; four non-changes recorded
- [`ADR-0032`](adr/0032-datetime-tool.md) — `datetime`: one read-only
  tool for the clock and calendar arithmetic (now/info/add/diff/
  convert); month-end normalization disclosed, business days refused;
  session-start date rides the system prompt cache-stably
- [`ADR-0033`](adr/0033-turn-observability.md) — turn observability:
  a stream heartbeat and stall warning, visible backoff retries, and
  ephemeral live thought summaries (displayed, never stored)
- [`ADR-0034`](adr/0034-cancellation-deadlock.md) — cancellation ends
  the call: process-group kill + WaitDelay (a grandchild holding the
  pipe hung timeout AND Ctrl+C), and a three-press last-resort exit
- [`ADR-0035`](adr/0035-opentelemetry-audit.md) — OpenTelemetry audit
  logging: OTLP log events, default off, global-config-only (the
  exporter is an egress channel), metadata never payloads, and
  telemetry that never hurts the session
- [`ADR-0036`](adr/0036-ask-user-tool.md) — `ask_user`: a structured
  mid-turn choice on the approval dialog's grammar; declining is
  information; every mode answers honestly; no free-text by design
- [`ADR-0037`](adr/0037-agentic-file-search.md) — `agentic_file_search`:
  delegated project search in an isolated child context — read-only
  allowlist, no recursion, labeled telemetry, ADR-0014 generalised
  from one file to one question
- [`ADR-0038`](adr/0038-risk-eval-instruction-context.md) — the
  auto-approve model tier sees the operator's typed request for a
  turn's first rounds: evidence-wrapped, misalignment escalates, late
  rounds fall back byte-identically to the call-only view
- [`ADR-0039`](adr/0039-integration-reload.md) — `/skills reload` and
  `/mcp reload` reuse the startup paths and the startup trust verdict;
  declarations and the system prompt follow; `--mcp on|off` for
  one-shot pipelines; reloads are audited
- [`ADR-0040`](adr/0040-round-limit-intervention.md) — the round limit
  becomes an intervention ladder: loop detector, progress review,
  operator dialog (auto mode continues itself on a confident verdict),
  a 3× cap no verdict can lift, and a stop message that teaches
  "continue" instead of /clear
- [`ADR-0041`](adr/0041-review-round-3.md) — whole-code review round 3:
  16 findings, 3 high (the child agent expanded model-authored @refs,
  the live region's tab width hole, a second stdin reader), plus the
  stall detector, ask dialog, and audit-gap fixes
- [`ADR-0042`](adr/0042-terminal-diagrams.md) — mermaid diagrams
  draw in the terminal: the measured-faithful types only (flowchart,
  ASCII sequence, ER), shapes normalized to boxes, a fidelity guard,
  the rest shown as source (the FIT rule and the prompt section were
  later removed by 0063)
- [`ADR-0043`](adr/0043-diagram-tool.md) — diagrams are drawn by a tool, not by rewriting what the model wrote (superseded by 0063)
- [`ADR-0044`](adr/0044-pre-tool-hooks.md) — operator pre-tool hooks: the org's guards survive the fallback
- [`ADR-0045`](adr/0045-transcript-approval-learning.md) — transcript-driven approval-rule learning: `/learn` proposes, the operator decides (withdrawn by 0049)
- [`ADR-0046`](adr/0046-mcp-description-risk-evidence.md) — MCP tool descriptions as risk-evaluation evidence: tell the evaluator what the operator already installed
- [`ADR-0047`](adr/0047-declared-purpose.md) — model-declared purpose on gated calls: the operator sees why, not only what
- [`ADR-0048`](adr/0048-learning-that-fires.md) — learning that fires on real usage: server-scoped MCP rules, and counting the answers people actually gave (withdrawn by 0049)
- [`ADR-0049`](adr/0049-learn-withdrawn.md) — `/learn` is withdrawn: confirmation was not a durable boundary for loosening
- [`ADR-0050`](adr/0050-risk-calibration.md) — the risk rulebook: layered guidance for the judge, and learning is one way to write it
- [`ADR-0051`](adr/0051-destructive-rewrite-floors.md) — whole-file rewrites that shrink are a red flag: the shrink guard, the regeneration rule, the compaction staleness notice, and the dialog size delta
- [`ADR-0052`](adr/0052-ignore-aware-navigation.md) — ignore-aware navigation: the walks skip generated/ignored content (built-in list + full gitignore semantics, no new dependency), search answers "where", list_tree budgets per directory (amends ADR-0013's premise)
- [`ADR-0053`](adr/0053-one-shot-approval-controls.md) — one-shot approval controls: `--auto` arms the ADR-0004 ladder headless (escalations become explained denials; the config key stays ignored — the grant belongs on the invocation), `--allow` grants per-run `"never"` entries through the normal policy build, and SessionStart reports the effective auto state
- [`ADR-0054`](adr/0054-risk-context-every-round.md) — the risk evaluator sees the operator's instruction in every round: ADR-0038 §3's cutoff measured against real transcripts (70% of evaluations and 63% of turns' terminal gated calls fell outside it) and removed; egress rubric, tiers, and confidence bar unchanged
- [`ADR-0055`](adr/0055-piped-stdin-as-data.md) — piped stdin in one-shot mode becomes a nonce-wrapped text attachment (the `@`-file lane), never prompt text: the `-p` string alone stays the risk evaluator's instruction channel; bounded read with a disclosed clip, binary skipped, terminal stdin never read
- [`ADR-0056`](adr/0056-stall-warning-threshold.md) — the stall warning cried wolf: a Gemini function call arrives as one whole part, so composing a large write/edit argument is measured minutes of dead wire (not one byte, let alone a chunk); the threshold moves 20s → 90s and nothing is added to the screen — the supplier's constraint belongs in this ADR, not in the status bar; `/riskbook learn` finally sets its suppression flag after `beginTurnStats`
- [`ADR-0057`](adr/0057-usage-accounting-records.md) — every model call leaves one `usage` record (source, model, prompt/output/thoughts/cached/total): the API never reports money, so cost is token counts × catalog price and the counts must be on disk at call time; risk and compaction spend used to die with the process, and thoughts bill as output while cached is a discounted share of prompt (both measured)
- [`ADR-0058`](adr/0058-session-work-directory.md) — a work directory per session (state root, keyed by session id, resume lands back in it): a sandbox write root and a second root of the file tools, exported as `GEMAGENT_WORK_DIR`. MCP results were the one tool output never bounded, which is why every file-mediated server grew a `workspace_root` — a server cannot know the model's context window; oversized results are now saved here rather than truncated, and non-text content (silently discarded until now) is saved for `view_image`
- [`ADR-0059`](adr/0059-workdirs-cleanup-command.md) — `gem-agent workdirs` list + `clean`: the cleanup half of ADR-0058's accumulation note (a report without a remedy trains people to ignore it). Confirmation is the default with deny-on-EOF, a live session's directory is never deleted (shared-flock probe on its transcript), and cleaning stays per-project and CLI-side — freeing disk must not require a model session
- [`ADR-0060`](adr/0060-deny-with-reason.md) — deny with reason, the `N` answer: the fixed denial text spent a model round fetching a reason the operator knew when they pressed `n`, and the denial's own function response is the one slot the API leaves open mid-round (ADR-0012). `n` stays a one-keystroke deny; denial results ship unwrapped by message provenance, never by content (a denial-shaped tool output stays wrapped); the reason lands in `gate_decision` but not in telemetry
- [`ADR-0061`](adr/0061-independent-runtime-promotion.md) — independent agent runtime: the backup charter is retired (real-world deployment outgrew it), drop-in compatibility stays the top requirement with an ecosystem-compatibility rationale, scope minimalism stands on its own charter instead of "20% of Claude Code", the drill becomes an on-demand health check, and gem-agent is promoted to cli-series by operator decision — the drill-based bar superseded, not passed
- [`ADR-0062`](adr/0062-delegation-first-exploration.md) — delegation-first exploration: 75 sessions / 788 tool calls held zero spontaneous agentic_file_search firings because the system prompt prescribed the manual list/search/read loop by name and never mentioned the tool — a description-level trigger cannot outcompete a prompt-level workflow. The prompt now routes exploration to delegation first (self-navigation is the known-target path), the description gains "trust the report", and the wiring is pinned by tests
- [`ADR-0063`](adr/0063-diagram-fences-render-in-place.md) — diagram fences render in place and the runtime says nothing about diagrams: two months measured the tool firing once while the model hand-drew box art instead (the fence prohibition over-generalized — a specific prohibition beside a vague recommendation reads as "rarely"). The fence path returns as a view-layer rewrite, the FIT gate is deleted (overflow wraps and loses nothing — measured), the wrongness guards stay, and an attempted draw that fails keeps the source plus a one-line reader-facing note
- [`ADR-0064`](adr/0064-first-message-argument.md) — a positional argument is the first interactive turn: `gem-agent "message"` submits it after the banner through the exact typed path (shell escape, slash, /skill, mentions), fires exactly once, composes with `--continue`/`--resume`/`--auto` unchanged, refuses combination with `-p`, and leaves ADR-0055's piped-stdin boundary untouched
- [`ADR-0065`](adr/0065-cancellation-in-process.md) — cancellation ends the call, part 2: the file walks consult the context and return a named partial result, a return-guaranteed floor under every tool call abandons a wedged call after a 1 s grace (audited as `abandoned`, counted on the exit receipt, recorded on a late return, and a late mutating return announced next turn; `ask_user` exempt), the three-press escape ladder reaches the plain REPL and `-p`, and the exit says when it is flushing audit events
- [`ADR-0066`](adr/0066-tool-prompt-usage-bucket.md) — the fourth bucket: the SDK defines `totalTokenCount` as the sum of four counts and ADR-0057's checksum had three, because its probe (the main loop) never enables a built-in tool — so every `web_search` / `web_fetch` record whose tool returned content failed the check it was meant to pass. `usage` records now carry `tool_prompt` (always written, zero included; a missing key marks a pre-0066 record), the checksum is `prompt + output + thoughts + tool_prompt == total`, and `model.usage` gains `tool_prompt_tokens`
- [`ADR-0067`](adr/0067-piped-stdin-wait-notice.md) — a one-shot run waiting on piped stdin says so: `-p` still reads a non-terminal stdin to EOF (a slow producer is never cut off), but a pipe still open after 2 s earns one stderr line naming both remedies (close the pipe, or launch with `< /dev/null`), and an announced wait is seen to end — the idle inherited pipe a scheduler or harness hands a child no longer passes for a hang
- [`ADR-0068`](adr/0068-telemetry-resource-declared-not-detected.md) — the telemetry client declares its Cloud Logging resource instead of probing for one: with `backend = "gcp"` the library's Logger classified the host by fetching from the GCE metadata server, and on a Mac that link-local fetch intermittently cost 4.5–7.2 s of silent startup (dial timeouts retried as transient while the kernel probed ARP for a neighbour that does not exist); the `global` resource the detection would have fallen back to is now declared, construction touches no network, and no wait notice is added because the wait was never a contract
- [`ADR-0069`](adr/0069-session-and-prompt-hooks.md) — session-start and prompt-submit hooks: `[[hooks.session_start]]` (sources `startup`, `resume`, `clear` on `/clear`; optional `matcher`) and `[[hooks.user_prompt_submit]]` (every turn that reaches the model) run on Claude Code's measured contract — the same stdin payloads, plain stdout or `hookSpecificOutput.additionalContext` as context, exit 2 or the JSON block forms refusing a prompt (erased, nothing recorded), a session start never blockable — and their output reaches the model as a data attachment beside the typed input (ADR-0055's lane, capped at 8000 runes, announced), never the system prompt and never the risk evaluator's trusted instruction channel
- [`ADR-0070`](adr/0070-skill-directory-and-shared-writable-roots.md) — a loaded skill names its directory, and the rule tier's writable places are the sandbox's: `load_skill(name)` and `/skill <name>` open with Claude Code's own line `Base directory for this skill: <dir>` (the symlink-resolved directory reads are already confined to), so a `SKILL.md` written to Claude Code's `SKILL_DIR` contract can run its scripts from gem-agent's global skill directory instead of searching the disk for them; `sandbox.ScratchDirs()` is the one list of scratch roots (`TMPDIR`, `/private/tmp`, `/dev`) that both the profile and the redirect rule read, so `2>/dev/null` no longer reads as a write outside the roots; and a read-only tree walk (`find`, `fd`, `du`, `rg`, recursive `grep`) starting at `/`, `~`, or an absolute path outside the roots is Review, not Safe — the read side of the sandbox is open by design, so the walk's cost is the mounts', and a cost is the model tier's to weigh
- [`ADR-0071`](adr/0071-session-identity-contract.md) — the session identity contract, aligned with Claude Code: a UUID v4 session id (timestamp ids still list and resume; `--resume` takes an unambiguous prefix), `/clear` starts a new session (`SessionEnd` then `SessionStart`), the project is a directory exported as `GEMAGENT_PROJECT_DIR` (no minted, stored or git-derived project identifier), and `[[hooks.session_end]]` joins the events; state layout, transcript format and payload shapes unchanged
- [`ADR-0072`](adr/0072-whole-code-review-round-4.md) — whole-code review round 4: forty-one findings from a maintainer pass and five independent reviewers, all but four fixed — a pre-tool hook's refusal no longer lets `view_image` attach its bytes; the rule tier reads a command the way bash runs it (newlines separate, `/bin/rm` is `rm`, `find -exec` and `sed -i` are not read-only, `<(…)` is dynamic); writes under `.git/` are Block and writes to `AGENTS.md`, `.mcp.json` and the other instruction/configuration files are Review only the operator answers (`OperatorOnly`); `/clear` rotates every consumer of the work directory and no longer freezes the TUI under a session hook; `session_start` on `/clear` has source `clear` (ADR-0071 §4 amended)
- [`ADR-0073`](adr/0073-capability-lanes-for-shell-exec.md) — capability lanes for `shell_exec`: the 103 findings of ADR-0072 classified by root cause; the kernel, not the command text, decides what a shell command may do — three Seatbelt lanes (`read` runs unasked and denies writes, network, preference writes, signals and IPC-capable programs; `write` asks and denies the persistent files; `operator` is the operator's alone), the rule tier reduced to a Block floor that can only escalate, one list each for scratch, persistent and credential paths read by the profile and the file tools, and `internal/archtest` pinning confined opens, bounded reads (`internal/bounded`) and the single decision point
- [`ADR-0074`](adr/0074-trust-is-granted-to-content.md) — trust is granted to content, not to a path: the agent-facing files a trusted project provides (instruction files, `.mcp.json`, `.gem-agent.toml`, skills) are pinned by SHA-256 when trusted and a changed one asks again before it is loaded (`-p` leaves it out and says so; `gem-agent trust --accept` re-pins); the write lane denies renaming the parent directories of existing persistent files; what a session added or changed among persistent files is reported at `/clear` and exit; `[approval].pin_trusted_files` opts out
- [`ADR-0075`](adr/0075-remote-fault-is-the-servers-state.md) — a remote tool's repeated identical error is the server's state, and the runtime says so: a failed MCP call renders with its provenance — the server's `isError` result, the server's JSON-RPC rejection, or a failure gem-agent could not complete — as a typed `RemoteError`; the executor counts consecutive identical error texts per tool within a turn and at 3 appends a runtime note outside the nonce tag (`runtime_note`, trusted by provenance like `denial`) stating what was measured — the arguments went out as written, the answer repeated — and asking the model to report to the user; `mcp_fault` transcript record and one operator notice per streak; no retry, no text classification, no prompt rule
- [`ADR-0076`](adr/0076-the-configuration-home-stays-readable.md) — the operator's configuration home stays readable to the lanes: a first draft denied `~/.config`, and its review found one deny breeding four re-allow rules, two ordering rules, symlink resolution and an operator-maintained list — the kernel sees the syscall, not the intent, and the tools the read lane exists for read their settings there; the behaviour is addressed at its trigger (ADR-0075) instead. ADR-0011 §3 amended: a skill installed for Claude Code is copied, not linked — `~/.claude` is a credential store the lanes cannot read and Seatbelt matches the resolved path; the `/skills` empty state prints the copy command
- [`ADR-0077`](adr/0077-absent-not-refused.md) — a tool the operator did not choose is absent, not refused: an MCP server is started outside every Seatbelt profile, so the lanes never reach it and the MITL gate is the only control — and the gate answers *when to ask*, not *what this session has*, which is the sentence an operator wants for a server offering `patch_*` beside `search_*`. And MCP declares no capability to filter on — a `Tool` is a name, title, description and schemas plus optional `annotations` a client must treat as untrusted — so matching the function name is not a preference among alternatives but the only expressible form of "give up write" — and a capability model would not settle it either, since against a Jira server the operator's sentence is "no new issues, but yes comments" and both are writes, so the function name is where the server's domain and the operator's intent already meet. One key, `[mcp] exclude`, removes a server or one of its functions (hand-written config, panel toggles into the machine-owned policy file, a project file that may only add); a server turned off is not started and its row still stands in the panel, from `.mcp.json`, to turn back on. An off tool is not declared and a call naming it is refused before the gate, existence-shaped to the model and recorded for the operator. Declaring a tool and refusing it is rejected: a prohibition re-read every round (ADR-0063's third path is `gh` in the write lane) in a state whose default readings are the ADR-0075 misreading. No named profiles — the predecessor that produced mcp-bridge carried 26 profile keys and used 5. No drift mechanism and nothing added to the startup banner: a report is not a control (this repository shipped four releases of explanatory banners), and a recorded baseline collapses on its own terms — the confirmation it compares against has no moment, an off server cannot be listed, a remote set changes server-side, and a degraded `tools/list` reads as mass departure then mass arrival. What covers a function that appears later is ADR-0008's default that every MCP tool asks
- [`ADR-0078`](adr/0078-the-banner-says-what-nothing-else-will.md) — the banner says only what nothing else will say: measured at 28 wrapped rows before the operator has typed anything, 11 of them one comma-joined MCP list and 4 one file path, and nobody decided to print them — the banner is assembled by teeing every startup write into it, so each feature's ADR added a line to a wall with no author, two of which end by naming the command that supersedes them. ADR-0077 §4 refused to add a drift report here because a report is not a control; this is that argument aimed at the lines that already exist. What stays is what nothing else says (build and model, the instruction files, a resume, everything abnormal); the servers, skills and memories become one counts row naming `/mcp /skills /memory`; the project path, the normal-case sandbox summary, the session-log path and the two self-superseding lines go, and the identity line drops the GCP project id it put in every screenshot
- [`ADR-0079`](adr/0079-a-notice-is-chrome-an-error-is-a-chain.md) — a notice is chrome, an error is a chain: eleven notices the agent writes mid-turn are in the ja/en catalog, moved with no ADR on the reading that ADR-0029 §3 already covered them; four were, the rest were new surfaces entering a closed enumeration. Worse, the rule written to justify the move — "a sentence carrying the operator's next command is chrome" — does not decide its own membership, since the compaction-failure string carries no command and is cataloged anyway. The discriminator is authorship: gem-agent composes the sentence → catalog (it may quote a cause verbatim), the sentence IS a returned `error` → English, because a translated frame around library innards is the mixing §3 removed. The next-command rule is about wording, and was never about language
- [`ADR-0080`](adr/0080-read-only-is-a-mode.md) — read-only is a mode, on its own axis (**Accepted**, implemented). Prose alone fails twice, measured 2026-09-09 against `gemini-3.8-flash`: a `make build` at a later turn whose input is 「続けて」 is approved at 0.95 while the identical call with the restriction in view escalates at 0.95 naming it — the model tier is not the weak part, the restriction just does not survive the turn — and a `write_file` with the restriction in *that very turn* is approved with the model tier never running, because an in-project write is rule-tier `safe`. ADR-0077 drew this line one level down: the gate says **when to ask**, not **what this session has**. `--auto` decides who answers the gate; this decides what the session may reach, so coupling them would leave the restriction inoperative in the two modes that need it most — manual, where the operator would otherwise answer *no* once per call, and `-p`, where there is nobody to answer. The axis is the session's **lane ceiling** plus the watcher that may raise it — two independent booleans, because as one tri-state, tightening destroyed the fact that the session was watching and lifting disarmed the watcher nobody asked to give up. A call above the ceiling is refused, never downgraded. The runtime may raise the ceiling and may **never** lower it — tightening only refuses more, while loosening is where a derived constraint becomes a derived permission. `[agent].read_only` sets the default and `--writable` / `--read-only` / `--auto-read-only` override it per invocation; `-p` has two forms only, since inferring a ceiling there would make a cron job's write access a function of its prompt's wording, and a configured `"auto"` is read as `"on"` because ignoring it would drop a restriction silently where nobody is watching. Enforcement is `LaneRead`, which already **is** the requested profile and is the one boundary probe-verified at startup: the mode caps the lane rather than swapping a profile, so nothing new is generated and nothing new can be got wrong. A write asks whether to lift — must-prompt, no `a`, `never` or model tier may answer, a decline suppresses further lift prompts that turn, and in `-p` the refusal is final. And the mode reaches the risk evaluator as the operator's **intent** rather than as enforcement, which is what covers MCP where no profile reaches: measured, it flips an otherwise-aligned vault write from approve 0.95 to escalate 0.95, escalates an external send at 1.00, leaves searches approved at 1.00, and — unlike state-shaped wording, which produced identical verdicts — never makes the model tell the operator a write was "not permitted" when nothing prevents it
- [`ADR-0081`](adr/0081-instruction-context-only-subtracts.md) — instruction context may only ever subtract, and by composition rather than by asking (**Accepted**, implemented, split from ADR-0080): the evaluator's instruction context is bidirectional today — ADR-0038 says alignment supports approval, and it was measured doing so, carrying a later-round `make build` to approval at 0.95 on a turn that said only 「続けて」. The channel is operator-typed text, which an attacker cannot write into directly, but an operator who pastes a log or an error message has carried attacker bytes into the one input this system trusts, and that is already true at one turn's width. So `approve = (the decision reached without instruction context) AND (the evaluation given it)`: no round at all for `safe` (the rule tier **is** the context-free decision, and instruction context never reaches those calls, so there is nothing to compose — corrected during implementation against a bullet inherited from ADR-0080's second draft), one when a baseline escalates and the aligned round is not worth running because context may not rescue anything, two otherwise — and the baseline, being context-free, is cacheable by tool and arguments. A composition rather than a prompt instruction, because "treat alignment as grounds to escalate, never to approve" is a string rule that has to hold against a model ignoring it — the reason ADR-0020 §4 removed memory writes from the model tier instead of telling it to be careful. Its floor is the context-free evaluator — the baseline round still carries the rulebook and an MCP tool's self-description, which are information about the call rather than context around it — it bounds the cage-as-licence reading ADR-0080 §5 looked for and did not find, and a change of mind is unaffected, since lifting never needs context to *create* an approval. Not measured: no probe has yet constructed the case this exists to stop
- [`ADR-0082`](adr/0082-the-model-tier-gets-its-own-model.md) — the model tier gets its own model slot (**Accepted**, implemented, amends ADR-0025 §2): `[model].risk` names the model that answers the risk evaluation, the progress review and the read-only watcher, `[model].risk_thinking` its level; unset, the tier rides the main model at `[model].thinking` as before, and once either key is set the slot runs on the same client with its own name and level and never inherits the main dial. Triggered by "could the on-device model do this?" — measured, no: zero guardrail refusals (the fallback's premise never fired) and nine confident false approvals in twenty-four, `curl \| sh` and an injected "pre-authorised" line among them. The same bench over fourteen Review-tier calls, configurations interleaved per case, settled two things: lowering the thinking level buys nothing on `gemini-3.8-flash` (7.6–8.9 s per call at 59 or 524 thought tokens — the endpoint's time), and the verdicts that matter survive on `gemini-3.7-flash` at `low` (3.65 s per decision, 0 wrong) but not on the no-thinking lite model (2.0 s, approves an Obsidian-vault write as "local" on every run) — and since the rule tier cannot read another server's effects, a judge that fails open on an MCP write has no floor under it. So: a slot, not a smaller model. Accounting names the slot (`usage` records, `evaluator_model`, `/usage`, `/info`, two `/settings` rows); a rejected slot model fails closed with the error in the reason, no silent fallback; the example config recommends the measured setting and the default is unchanged, since 3.7's retirement date is unannounced. Not measured: the progress review and the watcher on the slot model; compaction stays on the main backend
- [`ADR-0083`](adr/0083-mcp-tools-on-request-via-a-librarian.md) — MCP tools are advertised on request, chosen by a librarian that reads the catalogue so the main model does not have to (**Proposed**, not implemented): every request declares 243 tools — 67k tokens, 92% of a first-round prompt, and 68k characters of server-authored prose the main model reads unwrapped on every round, with ADR-0046's call-time red flag as the only defence. lagent ADR-0004's mechanics (registered is not advertised; `mcp_load`; preload; transcript replay) are adopted; its first-sentence catalogue is replaced by a one-shot side call holding the whole catalogue as nonce-wrapped data, which names the tools to load for a task the model describes (`find_tools`) and flags descriptions that address the model. Probed on the operator's real `tools/list` plus a planted lobbying server: 3.8-flash and 3.7-flash at `low` recover 17/18 prescribed servers, recommend the planted server 0/18 and flag it 18/18, in 3.5 s on a 19k-token catalogue; the lite model recommends the lobbyist and flags nothing, so the librarian defaults to the model tier's slot. Flagged means withheld (absent, not refused; `/mcp load` overrides), never "safe" — no gate reads the librarian's silence. `advertise = "all"` stays the default until firing, tokens and verdicts are measured in real sessions; ADR-0003 (system-prompt invariance) is measured to buy under a second on Vertex and is not taken up
- [`ADR-0084`](adr/0084-routes-not-rules.md) — the runtime supplies routes, not rules (**Accepted**, implemented; lagent ADR-0008 in gem-agent's terms): probed 2026-09-12, `go vet` in the read lane fails on the build cache under `~/Library/Caches`, so the prompt sent every build and test to the approval-gated write lane — two model-tier rounds or a prompt for what is inspection; the one-shot denial told the model to ask a user ADR-0065 says is not there; `list_tree dirs_only` called a flat directory empty and cost a `list_files` round; and a "denial is a decision — ask" rule stood where the runtime could carry the route itself. Now `GOCACHE` rides the session scratch in every lane (one function, Go only; not the operator's shared cache, which an unasked command could poison), so compile, vet and test run unasked in the read lane while a binary written into the project still needs `write`; an unattended denial names what still runs; a flat directory lists its files; the prompt's lane paragraph says what is true and the rule is gone. Not covered: module downloads (`~/go/pkg/mod` is not writable in any lane)
- [`ADR-0085`](adr/0085-credential-reads-are-operator-only.md) — credential paths are operator-only for the read tools too (**Accepted**, implemented): system risk review 2026-09-13, chapter 10 / R02 — `read_file .env` inside the project was Safe in every mode, its content flowed to the model and the transcript, and the one credential list (ADR-0073 §3) was enforced by the lanes, the write tools and the shell floor, never by the read tools. Now `read_file`, `view_image`, `read_document`, `file_info` and `summarize_file` on a path matching `sandbox.CredentialPath` (the `.env.example`/`.sample`/`.template`/`.dist` re-allow included, judged on the real path) are `Review` with `OperatorOnly`: must-prompt in the default gate and under `--auto` with the model tier unconsulted, not lifted by a session `a`, a `"never"` policy or `--allow`, denied in `-p`, refused by the file-search child's gate — the file-tool analogue of the operator lane being the only lane that may read credentials. `search_files`, `list_tree` and `list_files` do not prompt and hide nothing: ADR-0086 withdraws that, since the kernel lists names while refusing content. `@` attachments are unchanged. No list is added — one list, four enforcers
- [`ADR-0086`](adr/0086-the-kernel-reads-the-file.md) — the kernel reads the file: the credential list stops being a matcher the file tools carry (**Accepted**, implemented): the operator read the v0.78.0 release and asked whether the credential and environment protections are walking the road ADR-0073 withdrew — combinations growing, every hole answered with another pattern. The history says yes for the matcher: since ADR-0073 the list gained one entry and the matching rule changed six times, four of them in one release, each a spelling the previous rule had not considered. The file tools never made ADR-0073's move because they do not pass through Seatbelt, so their matcher is the boundary and a miss is the file's content in the transcript, not a missing prompt. Measured: under a profile carrying only the credential deny and the template re-allow, `cat .env` and `stat .env` are refused, `.env.example` reads, `grep -r` skips that one file, and `ls -a` still lists the name; `sandbox-exec` costs 6 ms over a bare spawn and 27 ms re-executing this binary, against a tool call that already costs a model round. So each covered read runs in a sandboxed child built from the same `internal/sandbox` list, a denied open becomes the operator's prompt and is re-issued in process on approval, and the matcher stops being load-bearing. Because the kernel lists names and refuses content, ADR-0085 §2 is withdrawn: names are no longer withheld, `credentialSkipNote` and the walk withholding are deleted, and the lagent/gem-agent contradiction over names versus counts disappears. Hard links, copies and secrets in unlisted files are written down as the ceiling rather than chased. The criterion left behind: an entry is cheap, a rule is the smell
- [`ADR-0087`](adr/0087-the-runtime-hides-only-its-own.md) — the runtime hides only its own: the environment scrub is withdrawn and the prefix rule is inverted (**Accepted**, implemented): the operator asked why a variable visible in the process space to any local program is hidden only when it goes through the agent. Measured, the scrub covers one child of six — write and operator shells, the `!` shell, MCP servers and hooks all get the operator's environment whole — and the one it covers passes `OPENAI_KEY`, `GH_PAT` and `GPG_PASSPHRASE` while catching `NPM_TOKEN`; the system risk review's chapter 10 had already written that one cannot explain this as "put a secret in an environment variable and it is protected". An allowlist is rejected as the repair: the set of variables a toolchain legitimately reads is as open-ended as the set of names a secret can have, so it only moves the unbounded list and makes it fail by breaking read-lane builds (ADR-0084's friction). Unlike ADR-0086 there is no kernel to move the judgment to, so the rule is deleted rather than relocated: the operator's environment is not touched. One set is bounded and the runtime enumerates it — its own namespace — so the inverted prefix rule is taken: everything `GEMAGENT_*` is removed from every child except the three exports ADR-0071 defines, a partition a test closes by requiring every `GEMAGENT_` literal to be classified. That is what R01 asked for, reached by construction rather than by recognising the word `key`

- [`ADR-0088`](adr/0088-the-caller-names-the-work-dir.md) — the caller names the work directory: `_meta` on every `tools/call` (**Accepted**, implemented): organization ADR-021 settled that a server producing files takes its destination as a per-call `work_dir` argument, and ten fleet servers came to require it — the contract places an obligation on the calling side too. Measurement of the four calling runtimes is why the argument exists at all: neither MCP `roots` nor the environment reaches half of them. The contract's second channel, the request's `_meta["jp.nlink/work_dir"]`, exists for the runtimes we write ourselves — it is schema-blind, so it rides every `tools/call` without knowing any tool's schema, and a model that forgets the argument still leaves the server a destination this session can read back. `GEMAGENT_WORK_DIR` (ADR-0071) does not serve here: it is an environment variable for child processes, not a protocol value, and reading it needs `${GEMAGENT_WORK_DIR}` in the registration entry, where the sibling runtime expands an undefined variable silently to the empty string. So `mcp.Client` carries the session work directory and attaches the key to `params._meta` on every call; a session with no work directory attaches nothing, because an empty hint is worse than none — a server would take it for an answer; the value is a `NewStdio` parameter rather than a global or a setter, since it does not change while the client lives and belongs at the call site; and the model's own argument always wins, `_meta` being read only when the argument was absent (ADR-021 §2's resolution order) — the runtime supplies a default, it does not overwrite intent. The ten in-house servers now write into this session's work directory even when the model omits `work_dir`; chrome-pilot's registration loses `--workspace-root ${GEMAGENT_WORK_DIR}`, the only line that made gem-agent's and lagent's `mcp.json` two separate files; and a server that does not know the key ignores it, which is what MCP's `_meta` is for

- [`ADR-0089`](adr/0089-inline-images-declare-their-height.md) — inline images declare their box: the counter is told, never measures (**Proposed**, not implemented; rewritten 2026-09-17 after an independent pass returned 17 findings in three classes — claims about adjacent code asserted without reading it, "measured" claims wider than the instrument, and a lane opened without enumerating the dimensions it opens): `emit` counts the physical rows of every line and the bottom pin rests on that count, but against x/ansi v0.11.6 `ansi.StringWidth` is 0 for iTerm2 `OSC 1337`, kitty `APC _G` and sixel `DCS q` alike while `ansi.Hardwrap` leaves all three byte-identical — and `physicalRows` floors at 1, so the shortfall is N-1, not N. What that costs was measured on the production path by `tools/pinprobe`, which drives the real model through the real emit path, with the control at the same fill in every run: on tmux 3.7c, which renders sixel, a full screen strands one frame per image (3 repeats, 3 stranded; the plain control is clean) while a screen not yet full takes no damage at all — and on iTerm2 3.7.2 five images, each undercounted by 11 rows, moved nothing, so the first draft's "the pin drifts by N-1 per image" is withdrawn. The code says why the regime matters: the pin's padding floors at zero once the screen is full. `rowprobe` separately established that the declared box is reserved exactly in both dimensions whatever the picture does inside it, and `pinprobe` showed the same on screen. So the emitter declares the box and `physicalRows` is told it — replacing its floor of 1, not adding to it — and the declaration covers COLUMNS as well, because `wrapForScrollback`'s strictly-narrower invariant is inert for exactly the lines whose width the counter cannot see. The source list collapses to ONE: the MCP intake already writes a tool's image into the work directory and hands the model a path (`cmd/mcpresult.go`), so the view layer draws a file this runtime named and wrote. A model-named path is rejected outright — it would be a view-layer open that never reaches `Agent.decide`, never runs in ADR-0086's sandboxed child, and is invisible to the credential list keyed on built-in tool name, which is the class ADR-0085/0086 repaired. Sixel is refused on the measurement rather than on a dependency rule that appears nowhere else in this repository and that `mermaid-ascii` already contradicts; drawing is TUI-only, with `-p` and the plain REPL naming themselves out; multiplexers default off because the one measured rendering a payload stranded frames. Left unmeasured and said so: whether kitty and Ghostty honour `r=`, whether Terminal.app implements any of the three, and the per-image cost

## History

Frozen audit trail of superseded documents. Empty so far — nothing has
been superseded yet. When something is, it moves here with a note saying
what replaced it, rather than being deleted: the discussion in a
superseded document is often the only record of why the current design is
shaped as it is.
