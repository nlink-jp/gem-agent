...
- [`ADR-0080`](adr/0080-restriction-outlives-the-turn.md) — a restriction the operator states must outlive its turn, and instruction context must only ever subtract (**Proposed**): measured 2026-09-09 against `gemini-3.8-flash`, "review only, do not change anything" fails twice. A `make build` proposed at a later turn whose input is 「続けて」 is approved at 0.95 ("Standard build command within project directory") while the identical call with the restriction in view escalates at 0.95 naming it — so the model tier is not the weak part, the restriction simply does not survive the turn — and a `write_file` with the restriction in *that very turn* is approved without the model tier running at all, because an in-project write is rule-tier `safe`. Feeding "the interaction history" is the wrong direction: history has three provenances, and both the standing restriction and its revision live only in what the operator typed, the one an injection attacker cannot write, while the model's own turns restore the self-justification ADR-0047 §3 stripped and tool results are attacker-writable outright; the channel is already separated in the code (`turnInput` is assigned in one place, from `Run`; `!` output enters through `AddContext`). But instruction context today is *bidirectional* — ADR-0038 says alignment supports approval and M1 measured it doing so — so the influence has to be made subtractive before the window may widen: `approve = (the decision reached without instruction context) AND (the aligned evaluation)`, which is one round for `safe` (the rule tier **is** the context-free decision), one when a baseline escalates, two otherwise. A composition, not a prompt instruction, because the property must hold against a model that ignores what it was asked; its floor is the pre-ADR-0038 evaluator, and a change of mind still works because lifting never needs context to *create* an approval — so no release path, and no auto-lift, has to exist. `--read-only` is rejected as answering a different question, and deriving a constraint object is rejected not because it would become a permission (a `safe`→`review`-only object is monotone — the first draft's reasoning was wrong) but because §1 arrives there with no new object to keep correct
