...
func TestRotateWorkDirMovesEveryConsumer(t *testing.T) {
	...
	systemRebuilt := false
	notes := rotateWorkDir(registry, shellExec, true, project, newWork, nil, nil, false, nil, func() { systemRebuilt = true })
	if len(notes) != 0 {
	...
	if notes := rotateWorkDir(registry, shellExec, true, project, "", nil, nil, false, nil, nil); len(notes) != 0 {
