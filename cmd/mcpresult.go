...
type mcpIntake struct {
	...
	// (ADR-0090 §1). Bytes, never the path: a read the view layer
...
func (in mcpIntake) render(server, tool string, blocks []mcp.Content) string {
	...
	for _, b := range blocks {
		...
		// Saved AND described: the one condition (ADR-0090 §2). A block
...
// the mismatch ADR-0090 §2 exists to prevent. What the bytes really are is
