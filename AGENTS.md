...
internal/trustpin/ content pins for the agent-facing files and the persistent-file
                   snapshot (ADR-0074); cmd/pins.go applies them to the grant
...
internal/termimg/  inline images in a DECLARED box (ADR-0089): capability
                   ...
                   model's show_image, or the operator's /show (ADR-0091,
...
- **The architecture tests are the rule** (ADR-0073 §4) —
  ...
  showing a cut
- **Trust is keyed on content** (ADR-0074) — `projectGrant` (cmd/pins.go)
  is what the loaders read: `trusted` from ADR-0023 and `excluded` from
  the pin check. Every loader of project content (`loadInstructions`,
  `discoverSkills`, `connectMCPServers`, the `.gem-agent.toml` policy
  half) takes the grant; never re-add a bare `projectTrusted` parameter
  (an archtest pins the call sites). `checkPins` prompts at an
  interactive startup only; `/clear` and the reloads exclude and note;
  with no pins recorded it pins only interactively (a `-p` run loads as
...
- **The model tier has its own backend and model name** (ADR-0082) —
  `a.tierBackend()` / `a.tierModel()` answer the risk evaluation, the
  progress review and the read-only watcher, and their `usage` records
  and `auto_decision.evaluator_model` carry `tierModel()`. Both resolve
  to the main backend/name while `[model].risk` and
  `[model].risk_thinking` are unset (the stored `riskBackend` /
  `riskModel` are nil/"" then — never read them directly); once either
  is set the slot is `backend.WithModel(...).WithThinking(...)`, in that
  order (`WithModel` resets the level by design), and never inherits
  `[model].thinking`. A new model-tier call site must use the accessors
...
- **kitty gets PNG, whatever arrived.** `f=100` is the protocol's PNG and it
  ...
  `kittyPxPerRow`, generous so the terminal still scales down) and
  re-encodes it as PNG; a picture it cannot decode gets no payload. iTerm2's
  `File=` names no format and draws JPEG itself, so that path sends the
  bytes as they are. Adding a decoder to `measure.go` means a new format
  reaches `kittyPNG` — it converts anything `image.Decode` can read.
...
- **A `/settings` MCP toggle reconnects ONE server** (`reconnectMCPServer`,
  cmd/mcp.go): the store's `reloadMCP(server)` removes exactly the
  registry names that server's attach recorded (`inv.registered` — not
  by prefix: `foo__bar` shares `foo`'s prefix, and a long name is
  truncated past it), then re-lists a running client (no respawn),
  ...
  files (`configs`, `configured`, `complete`, `summary`, `registered`). The whole-set
