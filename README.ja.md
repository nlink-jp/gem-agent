...
gem-agent trust                            # このプロジェクトの信頼状態と信用済みファイル
...
パイプは素の REPL に、`-p` は単発実行に（変更系ツールは拒否され、拒否文は
「あなたに聞け」ではなく何がまだ走るかをモデルに告げます —
...
**[読み取り専用はモード](docs/ja/reference/approval.ja.md)** — 承認とは
別の軸に立つセッションの上限です。`--read-only` はセッションを read
レーンに抑えるので、書込・メモリ保存・それ以上を求めるシェルコマンドは
ゲートに回されるのではなくその前に拒否され、セッションスクラッチの外は
何も変わりません。`--auto-read-only` は「何も変更しないセッション」を
依頼したときにランタイム側で ON にさせるもので、締める方向にしか
動きません。セッション中は `/readonly on|off` と `/readonly auto on|off`
がそれぞれを動かし、`--writable` は設定された上限からその実行だけ
降ります。状態はステータス行と起動バナーに出ます。上限を超えるコール
は「モードを解除するか」を訊きます — コール自体の承認とは別の問いで、
そちらはその後に改めて訊かれます（ADR-0080）。
...
など指示・設定ファイルへの編集は必ずあなたに確認し、資格情報名のファイル — `.env`・
鍵・資格情報ストア — の読取は全モードであなたに確認して `-p` では拒否。読取自体は
資格情報を開けないサンドボックス下の子で走るので、問いを上げるのは一覧、拒むのは
カーネル。walk はそれらを列挙し、読めなかったファイルを名指し、信頼したプロジェクトの指示・
設定ファイルは内容でピン留めされ変化すれば読み込む前に再確認）、文字列ではなく宣言した
Seatbelt レーンで判定されるシェルコマンド（read レーンは無確認で走り —
compile・vet・test も含む、ツールチェインのキャッシュは全レーンでセッション
スクラッチに乗る — write レーンは `AGENTS.md` や `.git/config` に触れられず、
operator レーンはあなただけのもの。ADR-0084）、MCP サーバーが 3 回続けて同じエラーを返したときのランタイム
注記（モデルはそのエラーが誰の言葉かを知らされ、調査ではなくあなたへの
報告を求められる）、スコープ対応解決と
...
`[mcp].advertise = "on-request"`（opt-in）は全サーバーを接続したまま、ツールを
ロードされてからモデルに宣言する — モデルが `find_tools`（目録全体を読んで課題に
合うツールを名指す司書の側呼び出し）か `mcp_load` で、あるいはあなたが
`preload`・`--allow`・`/mcp load` で — そして、ツールの説明ではなくアシスタントに
話しかける説明文のツールを差し止める（ADR-0083）。
...
gem-agent は Apple Silicon の macOS だけを対象にビルド・署名・テストしており、
上で述べた保護は macOS のカーネルサンドボックス（`sandbox-exec`）に立っています。
起動のたびに gem-agent はそのサンドボックスを実プローブで測定し、結果でコマンドの
扱いが決まります:

- **検証済み** — 起動バナーにサンドボックスの警告が無い: read レーンのコマンドは
  無確認で走り、write レーンのコマンドは確認を求め、どちらもカーネルが範囲を縛ります。
- **未確認** — バナーに `sandbox unverified: …` か `sandbox: DISABLED` が出る:
  全シェルコマンドが実行前にあなたに確認し、コマンドとあなたのマシンの間に他の
  防壁はありません。どちらかはバナーが告げ、`/settings` が測定結果を求めに応じて示します。

別のプラットフォーム（Linux・WSL・Windows）向けに再ビルドしたソースのコピーには、
背後にカーネルサンドボックスがありません。上の警告なしに起動するなら、サンドボックス
の確認処理が改変されており、この README が説明する挙動ではありません — 何が表示されても
未確認として扱ってください。そうしたビルドはここではサポートしません。`internal/sandbox`
配下のテストが、あらゆる移植が満たすべき基準です。
