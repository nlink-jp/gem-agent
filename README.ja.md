...
返した理由を明示して「ブロックされた」と報告**します。

セキュリティ業務は、ごく普通の資料（インシデント対応手順書、フィッシング
分析など）でも調整可能な *dangerous content* フィルタに引っかかります。
`[model].safety` で調整可能な 4 カテゴリの閾値を変更できます:
...
なお Vertex には**どの設定でも無効化できないフィルタ**があり、そちらによる
ブロックも同様に報告されますが、対処は言い換えか `/clear` になります。
