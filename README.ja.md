...
**複数行入力**は次の 3 通りのいずれかで行えます。Shift+Enter を採用していないのは、
多くのターミナルがこれを通常の改行コードとして送るため、送信と区別できないからです:

| 方法 | 備考 |
|---|---|
| `Ctrl+J` | 常に使える |
| `Option`/`Alt` + `Enter` | チャット UI で一般的 |
| 行末に `\` を置いて `Enter` | シェルの慣習 |

複数行のペーストもそのまま使えます（ペースト全体が 1 メッセージとして入力
ボックスに入り、行ごとに LLM コールが飛ぶことはありません）。
