...
- **The MCP intake no longer draws. The model shows, and so can you**
  ([ADR-0091](docs/en/adr/0091-showing-is-an-act-of-output.md)). An MCP
  ...
  four can emit an image block and none sets an audience. So v0.83.0's rule,
  ...
  **This changes behaviour released in v0.83.0.** What reaches your screen is
  now authored:
...
- **MCP servers are spawned and listed at once instead of one after another.**
  ...
  and a slow day at either landed on it in full. Startup now costs the slowest
  server rather than the sum. Only the waiting is concurrent: attaching still
  runs in configured order, so the registry, the catalog and the warnings read
  exactly as before.
...
- **The search walk was unprotected wherever the file-read cage could
  not be installed.** ADR-0086 §5 said the file tools fall back to
  ...
  with nothing in between — weaker than 0.78.0, which withheld the
...
- **The file-read cage never switched on in 0.79.0.** Its startup probe
  ...
  holding a credential-named file) — restart gem-agent to try again`. Nothing
  was less safe than before the release: the reads fell back to the
  in-process credential check, which is what 0.79.0 did. But the release's
...
- **The degradation warning told the operator to do something that
  ...
  gem-agent itself rather than by the kernel — and points at `/settings`,
...
- **The kernel reads the file** (ADR-0086). The file tools never made
  ...
  `file-read-data`, as the third bullet of this release says; this
...
- **Review fixes to the kernel read path** (ADR-0086, independent
